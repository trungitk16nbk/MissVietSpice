...
class QuantityInput extends HTMLElement {
  ...
  validateQtyRules() {
    ...
    if (this.input.min) {
      ...
      const buttonMinus = this.querySelector(".qty-bt[name='minus']");
      buttonMinus.classList.toggle('disabled', value <= min);
    }
    if (this.input.max) {
      const max = parseInt(this.input.max);
      const buttonPlus = this.querySelector(".qty-bt[name='plus']");
...
class VariantSelects extends HTMLElement {
  ...
  updateImage() {
    ...
    if (this.currentVariant && this.currentVariant.featured_image) {
      jQuery('#zoom_mw-'+ this.currentVariant.featured_image.product_id +'').attr('src',this.currentVariant.featured_image.src);
      jQuery('.product-img').find('figure').css('background-image', 'url(' + this.currentVariant.featured_image.src + ')');
      jQuery('.product-single__thumbnail').removeClass('active');
      jQuery('#pro-detail-slider-'+ this.currentVariant.featured_image.product_id +' a[data-image-id="' + this.currentVariant.featured_image.id + '"]').addClass('active');
    }
  }
  updateDiscount() {
    ...
    if(this.currentVariant.price < this.currentVariant.compare_at_price) {
      $ProductDiscount.html(((this.currentVariant.compare_at_price - this.currentVariant.price)*100.0/(this.currentVariant.compare_at_price)).toFixed());
  ...
  updateVariantStatuses() {
    ...
    const inputWrappers = [...this.querySelectorAll('.op-field')];
...
customElements.define('pr-option', VariantRadios);
...
class DetailsModal extends HTMLElement {
  ...
  isOpen() {
    return this.detailsContainer.hasAttribute('opena');
  }

  onSummaryClick(event) {
    event.preventDefault();
    event.target.closest('details').hasAttribute('opena')
      ? this.close()
      : this.open(event);
  }
  ...
  open(event) {
    ...
    event.target.closest('details').setAttribute('opena', true);
  ...
  close(focusToggle = true) {
    ...
    this.detailsContainer.removeAttribute('opena');
